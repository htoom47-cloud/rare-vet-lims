...
assert.ok(/router\.post\('\/invoices\/zatca-submit-all'/.test(routesSrc));
assert.ok(routesSrc.indexOf('/invoices/zatca-submit-all') < routesSrc.indexOf('/invoices/:id/zatca-submit'));
assert.ok(/router\.post\('\/invoices\/:id\/zatca-submit'/.test(routesSrc));
assert.ok(/submitInvoiceById/.test(zatcaSrc));
assert.ok(/submitEligibleInvoices/.test(zatcaSrc));
assert.ok(/manual = false/.test(zatcaSrc));
assert.ok(/already_reported/.test(zatcaSrc));
assert.ok(/reported_invoices/.test(zatcaSrc));
