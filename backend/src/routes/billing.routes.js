...
router.post('/zatca/production-csid', authorize(PERMISSIONS.BILLING_CREATE), async (req, res, next) => {
  try {
    const data = await zatcaService.requestSandboxProductionCsid(req.user.id);
    res.json({ success: true, data });
  } catch (err) { next(err); }
});
