...
const { mapDiasysDeviceCodeToLims, isUreaLimsCode } = require('../utils/diasys-chem-map');
const { findChemUreaParameter } = require('../utils/urea-lims-parameter');
...
const resolveSystemParameterId = async (testCode, systemParameterCode) => {
  ...
  if (isUreaLimsCode(systemParameterCode)) {
    const urea = await findChemUreaParameter(testCode);
    if (urea) return urea;
  }
