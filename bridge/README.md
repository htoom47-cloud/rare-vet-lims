...
- يحدّث المعدلات الطبيعية الافتراضية للإبل (`sync-norma-refs`)
- **يجلب معدلات Norma لكل نوع حيوان** من آخر استيراد HL7 (`pull-norma-species-refs`)

### جلب المعدلات لكل حيوان (مرة واحدة لكل نوع)

1. سجّل عينة CBC في LIMS لحيوان من النوع المطلوب (إبل / خيل / غنم / ماعز).
2. على Norma اختر **نفس نوع الحيوان** قبل تشغيل التحليل.
3. أرسل النتائج — تُحفظ المعدلات تلقائياً من OBX-7.
4. لإعادة المزامنة يدوياً (Render Shell):

```bash
cd backend && node src/scripts/pull-norma-species-refs.js
```

**لا تحتاج** تشغيل أوامر يدوياً على Render بعد كل تحديث — يتم تلقائياً عند التشغيل.
